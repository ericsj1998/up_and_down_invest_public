...
```
src/updown/
├── common/          도메인 모델·설정·로깅·비용 테이블 · **시장 능력표**(`domain/capabilities.py`) · **마켓 캘린더**(`domain/session.py`)
├── marketdata/      시세/주문 I/O — BrokerAdapter 뒤로 브로커 차이를 숨긴다 · 재무제표 어댑터(`fundamentals/` · EDGAR)
...
- 🔴 **시장 차이는 능력표가 말한다** (T237 결정 ② · 2026-09-09). 배율·숏·수량 단위·결제·펀딩·24h·호가 규칙은
  `config/markets.yml`(`capabilities_of`), 휴장·조기마감·개장 시각은 `config/market_sessions.yml`(`MarketCalendar`)이
  단일 출처다. 엔진·원장·러너·화면은 **시장 이름으로 분기하지 않고** 이 표를 읽는다 — `if market == "NASDAQ"` 이
  상위 계층에 나타나면 추상화가 샌 것이다. 주식과 코인은 **같은 세션 엔진**을 돈다(주식 = 배율 1 · 롱 온리 · 정수 주 · 장중만).
...
| 10 | ~~**롱 온리**~~ → **양방향 허용** (2026-08-17 사용자 확정). 숏은 **모의 라이브(T13)와 Gate.io 선물** 경로에서만 열린다. 현물(업비트)은 여전히 롱 온리이며, 숏 계획을 현물 어댑터로 보내면 즉시 예외다. **주식 현물(토스 · KRX/NASDAQ/NYSE)도 롱 온리** — 능력표 `short_allowed: false` 가 세션에서 숏 후보를 버리고(`short_blocked`) 페이퍼 어댑터가 보유 초과 매도를 거절한다 (T239·T240) | §12.8 개정 |
