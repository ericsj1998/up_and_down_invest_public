...
| ![AI 투자 어시스턴트](docs/readmeimage/ai_assistant_chat_1.png) | **AI 투자 어시스턴트**. 예시 질문 카드와 모델 선택이 있다. 오른쪽의 펀드 설정 마법사(동의, 자본, 성향, 매매 설정, 검토)는 AI 가 아니라 사전 설정이고, 숫자는 모두 과거 실측이다. |
| ![도킹 채팅과 펀드](docs/readmeimage/ai_assistant_chat_2.png) | **도킹 채팅**. 어시스턴트는 어느 화면에서든 가장자리에 붙는다. "200만원으로 미국주식 시작하려는데" 같은 질문에 도구로 사실을 읽어 답하고, 매매법의 과거 실측(기간, 손익, MDD, 추천 여부)을 숨기지 않는다. |
