...
```
 거래소 캔들 ──▶ 수집·무결성 검사 ──▶ PostgreSQL(candles)
                                          │
              ┌───────────────────────────┴───────────────────────────┐
              │  analysis  지표 · 구조물 · 셋업 탐지(플러그인) · 플레이북  │  ← "제안"만
              └───────────────────────────┬───────────────────────────┘
                                          ▼
              decision  RiskManager — 손절·익절·수량을 확정 (단일 출처)
                                          ▼
   ┌── 백테스트(봉인 구간) ──┐  ┌── 모의 라이브(테스트넷) ──┐  ┌── 실계좌 ──┐
   │   Session.step()       │  │   Session.step()          │  │ Session.step() │   ← 같은 코드
   └───────────┬────────────┘  └─────────────┬─────────────┘  └──────┬────────┘
               ▼                             ▼                       ▼
            원장(Ledger) ── 30초 체결 흡수 · 120초 거래소 대조 · 손절 감시 · 펀딩 귀속
               ▼
   리포트(누적 손익 · 결과 분포 · 판별 · 자산 일일 스냅샷 · 메일) · 콘솔(React)
```
