...
| AI | 채팅 도구 15개, MCP 로 13개 개방, 차트 분석 참가자 3(서로 다른 문맥으로 같은 봉을 보고 채점된다) |
...
사용자를 받는다. PostgreSQL 은 원장·봉·재무 사실·AI 실험 기록·감사 로그를, Redis 는 리더 락과 캐시를 담당한다.
...
한 모델이 전부 하지 않는다. 역할마다 주체가 다르고, 그 경계를 코드가 지킨다.

계획과 종합은 모델이 한다. 사실은 도구가 준다. 도구는 전부 우리 코드이고 DB 와 거래소를 읽는다.
손절과 수량의 확정은 RiskManager 가 한다. 모델이 낸 숫자는 그 자리에 못 들어간다.
최종 판단은 사람이 한다. AI 모듈은 결정과 집행 모듈을 import 조차 못 하고, 그것을 계약 검사가 CI 에서 막는다.

차트 분석 주문에서는 이 구조가 한 걸음 더 간다. 같은 순간의 봉을 고정해 놓고 **참가자 셋이 각자 답한다.**
규칙 엔진, 차트만 본 AI, 차트와 우리 근거를 함께 본 AI 다. 셋은 서로 다른 문맥을 받고 동시에 돈다.
답을 하나로 뭉개지 않고 나란히 두며, 시간이 지나면 같은 기준으로 채점해 성적표에 쌓는다.

다음 단계는 **역할을 나눈 에이전트**다. 뉴스를 읽는 에이전트는 봉을 보지 않고, 구조를 읽는 에이전트는 기사를 보지 않으며,
중재자는 원자료를 다시 읽지 않고 셋이 낸 근거만 받아 엇갈린 지점을 남긴다. 설계는 T277에 있고 아직 구현 전이다.

에이전트 프레임워크(LangGraph)는 쓰지 않는다. 우리 구조가 부채꼴로 펼쳤다 모으는 모양이라 분기도 순환도 없고,
프레임워크가 끌고 오는 모델 클라이언트가 바깥 호출 층을 지나지 않기 때문이다.
근거와 다시 꺼낼 조건 셋은 [ai_agents_and_mcp.md](docs/architecture/ai_agents_and_mcp.md)에 적어 두었다.
