...
class TestStanding:
    def test_fresh_pending_is_pending(self) -> None:
        ...
        # 기본 유예는 한 달이다 (사용자 2026-09-08) — 29일은 아직 대기
        assert _standing(age=timedelta(days=29, hours=23)) is Standing.PENDING

    def test_pending_past_the_grace_is_held(self) -> None:
        assert timedelta(days=30) == HOLD_AFTER
        assert _standing(age=HOLD_AFTER) is Standing.HELD
        assert _standing(age=timedelta(days=45)) is Standing.HELD
    ...
    def test_release_window_keeps_pending_then_holds_again(self) -> None:
        assert (
            _standing(age=timedelta(days=40), released=T0 + timedelta(hours=1)) is Standing.PENDING
        )
        assert _standing(age=timedelta(days=40), released=T0) is Standing.HELD
        assert _standing(age=timedelta(days=40), released=T0 - timedelta(days=1)) is Standing.HELD
