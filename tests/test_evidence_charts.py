...
class TestWriter:
    def test_write_then_load_round_trip(self, tmp_path: Path) -> None:
        """쓰기(견본)와 읽기(화면)가 같은 배치를 말한다 — 어긋나면 봉이 엉뚱한 종목에 붙는다."""
        ...
        ec.write_backtest_store(
            ...
            equity=([100, 200], [1.0, 1.1]),
        )
        store = ec.Store.load(path)
        a = ec.bt_candles(store, "A")
        b = ec.bt_candles(store, "B")
        assert a.t == [100, 200] and a.h == [11.0, 12.0] and a.c[1] == 11.5
        assert b.t == [100] and b.l == [0.5]
