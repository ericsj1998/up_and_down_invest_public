...
Note:
    ...
    ⚠️ 리비전 id 는 **32자 이하**여야 한다 — `alembic_version.version_num` 이 `varchar(32)` 라
    `0128_accounts_sessions_invalid_before`(37자)로는 UPDATE 가 잘려 마이그레이션 컨테이너가
    죽고 api 가 안 떴다(2026-09-11 로컬 재빌드에서 잡음).

Revision ID: 0128_sessions_invalid_before
...
revision: str = "0128_sessions_invalid_before"
