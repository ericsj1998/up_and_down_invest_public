      ...
      <div className="flex flex-col gap-6">
        ...
        {sample ? (
          ...
          <div
            ...
            role="alert"
          >
            견본 근거를 못 읽었다 — {sampleError || error}
          </div>
        )}
...
function StandardOnlyNotice({ hasBundle }: { hasBundle: boolean }) {
  return (
    <div className="notice warn" role="status" style={{ fontSize: 14 }}>
      🔔 <b>감사 권한이 없어 표준(견본) 매매법의 결과만 보인다.</b> 아래는
      이동평균 교차 견본을 실계좌와 같은 엔진으로 돌린 것이다 — 엣지 주장이
      아니라 플랫폼이 어떻게 재는지 보여 주는 표본이다.{" "}
      {hasBundle
        ? "실제 운용 매매법의 성적은 관리자가 계정 화면에서 감사 권한을 주면 여기서 그대로 보인다."
