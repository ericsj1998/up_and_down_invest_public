...
export interface LiquidationSummary {
  ...
  /** 청산 / 전체 매매 (%). */
  pct: number;
  by_symbol: { symbol: string; liquidations: number; trades: number; pct: number; futures: number }[];
  by_scenario: { scenario: string; liquidations: number; futures_hit: number; futures: number }[];
}
...
export interface SyntheticDetail {
  ...
  liquidations: number;
  symbols: string[];
  equity: { t: number[]; fund: number[] | null; market_log: number[]; step: number };
...
export interface BacktestSummary {
  ...
  by_reason?: Record<string, number>;
  trades_by_symbol?: Record<string, number>;
  documented?: { note: string; total_pct: number; mdd_pct: number; years: number; trades_count: number };
  generated?: string;
  missing?: boolean;
  reason?: string;
}
...
export const syntheticList = () => request<SyntheticList>("/evidence/synthetic", undefined, 60_000);
export const syntheticDetail = (k: number) => request<SyntheticDetail>(`/evidence/synthetic/${k}`, undefined, 60_000);
...
export const backtestList = () => request<{ backtests: BacktestSummary[] }>("/evidence/backtest", undefined, 60_000);
export const backtestDetail = (id: string) => request<BacktestDetail>(`/evidence/backtest/${id}`, undefined, 90_000);
export const backtestCandles = (id: string, symbol: string) =>
  request<CandlesResponse>(`/evidence/backtest/${id}/candles?symbol=${encodeURIComponent(symbol)}`, undefined, 90_000);
