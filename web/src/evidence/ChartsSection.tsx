...
export function ChartsSection() {
  ...
  const [tab, setTab] = useState<Tab>(fromUrl !== null ? "synthetic" : "backtest");
  const [list, setList] = useState<SyntheticList | null>(null);
  const [error, setError] = useState("");
  const [k, setK] = useState<number | null>(fromUrl !== null && Number.isInteger(Number(fromUrl)) ? Number(fromUrl) : null);
  ...
  useEffect(() => {
    ...
    syntheticList()
      .then((got) => {
        ...
        if (k === null) {
          const first = got.futures.find((f) => (f.liquidations ?? 0) > 0) ?? got.futures[0];
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-2" role="tablist" aria-label="차트 종류">
        <button type="button" role="tab" aria-selected={tab === "backtest"} className={tabClass(tab === "backtest")} onClick={() => setTab("backtest")}>
          백테스트 (실측 봉)
        </button>
        <button type="button" role="tab" aria-selected={tab === "synthetic"} className={tabClass(tab === "synthetic")} onClick={() => setTab("synthetic")}>
          합성 45미래
        </button>
      </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          {list ? (
            <div className="flex flex-wrap items-center gap-2 text-xs">
              <label htmlFor="future-pick" className="text-blue-gray-600 dark:text-blue-gray-300">
                미래 고르기
              </label>
              <select
                id="future-pick"
                  ...
                  <optgroup key={scenario} label={scenario}>
                    {rows.map((f) => (
                      <option key={f.k} value={f.k}>
                        #{f.k} s{f.seed} · {pct(f.total_pct)} · MDD {mdd(f.mdd_pct)}
                        {(f.liquidations ?? 0) > 0 ? ` · 강제청산 ${f.liquidations}` : ""}
                      </option>
                    ))}
                  </optgroup>
                ))}
                  ...
                  .map((f) => (
                    <button
                      ...
                      className={`rounded-md border px-2 py-0.5 text-[11px] ${
                        f.k === k ? "border-loss bg-loss text-white" : "border-loss/40 text-loss hover:bg-loss-wash"
                      }`}
                      onClick={() => setK(f.k)}
              ...
              <span className="text-blue-gray-400">
                {list.futures.length}미래 · 블록 {list.block ?? "—"} · 생성 {list.generated?.slice(0, 10) ?? "—"}. ③ 절의 점·행을 눌러도 같은 화면이다.
              </span>
            </div>
          ) : (
          ...
          {k !== null ? (
            <div className="rounded-xl border border-[#8957e5]/40 bg-[#8957e5]/5 p-4 dark:bg-gray-900">
              <SyntheticDetailPanel pick={{ k }} onClose={() => setTab("backtest")} />
            </div>
          ) : null}
        </div>
      ) : null}
