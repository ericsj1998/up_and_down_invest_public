...
import { fmtPrice, reasonLabel, sideLabel, type TradeMark } from "../chart/trades";
...
export function BacktestPanel() {
  ...
  useEffect(() => {
    ...
    backtestList()
      .then((got) => {
        ...
        const first = got.backtests.find((b) => !b.missing);
  ...
  const symbolTrades = useMemo<TradeMark[]>(() => (candles ? toTradeMarks(candles.trades) : []), [candles]);
  const allTrades = useMemo<TradeMark[]>(() => (detail ? toTradeMarks(detail.trades) : []), [detail]);
  ...
  const rows = useMemo(() => {
    const filtered = allTrades.filter((t) => t.symbol === symbol && (reason === "all" || t.reason === reason));
  ...
  const equityLines = useMemo(
    () =>
      detail && detail.equity.value
        ? [{ label: "자본 (×)", color: "#0f7b6c", values: toMultiple(detail.equity.value), width: 2 as const }]
        : [],
    [detail],
  );
  const liqMarks = useMemo(
    () => allTrades.filter((t) => t.reason === "liq").map((t) => ({ time: t.closedTs, label: `청산 ${t.symbol.replace("USDT", "")}` })),
    [allTrades],
  );
    ...
    return (
      <Card className="border border-dashed border-blue-gray-200 shadow-none dark:border-gray-700 dark:bg-gray-900">
        <CardBody className="p-4 text-sm text-blue-gray-600 dark:text-blue-gray-300">
          저장된 백테스트 차트 자료가 없다 — 연구 PC 에서 `backtest_paths.py` 를 돌려 `config/evidence/backtest_*.json` 을 만든다.
        </CardBody>
      </Card>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-2" role="tablist" aria-label="백테스트">
        {list.map((b) => (
          <button
            key={b.id}
            type="button"
      ...
      {!detail ? (
        <p className="faint">백테스트 자료(매매 {list.find((b) => b.id === id)?.trades_count?.toLocaleString("ko-KR") ?? "—"}건)를 읽는 중…</p>
      ) : (
        <>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
            <Fact label="구간" value={`${detail.years ?? "—"}년 · ${detail.frame ?? ""}`} />
            ...
            <Fact label="매매 · 강제청산" value={`${(detail.trades_count ?? 0).toLocaleString("ko-KR")} · ${detail.liquidations ?? 0}건`} />
            <Fact label="칼마" value={detail.calmar?.toFixed(2) ?? "—"} />
          </div>
          {detail.redacted ? <AuditNotice what="전체 수익률 · CAGR · 칼마 · 자본 곡선 · 매매별 손익" /> : null}
          {detail.documented ? (
            <p className="rounded-lg bg-blue-gray-50/60 px-3 py-2 text-xs leading-relaxed text-blue-gray-600 dark:bg-gray-800 dark:text-blue-gray-300">
              <b>2026-09-06 재실행값이다.</b> 문서의 E1 은 {pct(detail.documented.total_pct)} · MDD {mdd(detail.documented.mdd_pct)} ·{" "}
              {detail.documented.years}년 · 매매 {detail.documented.trades_count.toLocaleString("ko-KR")} — 다른 이유: {detail.documented.note}
            </p>
          ) : null}
          ...
          <Card className="border border-blue-gray-100 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <CardBody className="p-4">
              <div className="mb-1 text-sm font-semibold text-blue-gray-800 dark:text-blue-gray-100">자본 곡선 (로그 배수 · 붉은 네모 = 강제청산)</div>
              {detail.equity.value ? <EquityChart t={detail.equity.t} lines={equityLines} marks={liqMarks} /> : <AuditNotice what="자본 곡선" />}
            </CardBody>
          </Card>

          <Card className="border border-blue-gray-100 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <CardBody className="p-4">
              <div className="mb-2 flex flex-wrap items-center gap-1" role="tablist" aria-label="종목">
                {(detail.symbols ?? []).map((s) => (
                  ...
                  >
                    {s.replace("USDT", "")} <span className="text-blue-gray-400">{detail.trades_by_symbol?.[s] ?? ""}</span>
                  </button>
                ))}
              </div>
              {candles ? (
              ...
              <p className="mt-1 text-[11px] text-blue-gray-500 dark:text-blue-gray-300">
                아래 표의 행을 누르면 그 매매로 이동한다 — 손절선·진입선·청산선과 손익 영역은 선택한 매매 것이다. 마우스 휠로 확대·이동.
              </p>
              <div className="mt-3">
          ...
          <Card className="border border-blue-gray-100 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <CardBody className="p-0">
              <div className="flex flex-wrap items-center justify-between gap-2 px-4 pt-4">
                <Typography variant="small" className="font-semibold text-blue-gray-800 dark:text-blue-gray-100">
                  매매 {rows.length.toLocaleString("ko-KR")}건 — {symbol.replace("USDT", "")}
                ...
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>
                    {shown.map((t) => (
                      ...
                      >
                        <td className="px-3 py-1.5 font-mono text-xs">{fmtTs(t.openedTs)}</td>
                        <td className="px-3 py-1.5 font-mono text-xs text-blue-gray-500">{t.leg}</td>
                        <td className={`px-3 py-1.5 text-xs font-semibold ${t.side === 1 ? "text-gain" : "text-loss"}`}>{sideLabel(t.side)}</td>
                        <td className="px-3 py-1.5 text-right font-mono text-xs">{fmtPrice(t.entry)}</td>
                        <td className="px-3 py-1.5 text-right font-mono text-xs text-loss">{fmtPrice(t.stop)}</td>
                        <td className="px-3 py-1.5 text-right font-mono text-xs">{fmtPrice(t.exit)}</td>
                        <td className={`px-3 py-1.5 text-xs ${t.reason === "liq" ? "font-semibold text-loss" : ""}`}>{reasonLabel(t.reason)}</td>
                        <td className="px-3 py-1.5 text-right font-mono text-xs">{holdLabel(t.openedTs, t.closedTs)}</td>
                        <td className={`px-3 py-1.5 text-right font-mono text-xs ${t.pnl === null ? "text-blue-gray-400" : t.pnl >= 0 ? "text-gain" : "text-loss"}`}>
                          {t.pnl === null ? "🔒" : `${t.pnl >= 0 ? "+" : ""}${t.pnl.toLocaleString("ko-KR", { maximumFractionDigits: 1 })}`}
                        </td>
                      </tr>
                    ))}
                  </tbody>
