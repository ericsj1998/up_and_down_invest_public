...
/* ══ 매매 라벨 (2026-09-03 · 규칙 역산용) ═══════════════════

   ⚠️ 표시는 **정답지가 아니라 가설**이다 (절대 규칙 #11 예외 조건: 그 판단이
   규칙으로 환원되어 코드에 남는가). 뽑은 규칙은 표시하지 않은 구간에서 검증한다. */

export type LabelMark = {
  kind: "long" | "short" | "swap" | "evidence" | "level";
  ts: number;
  price: number;
  note?: string;
};

export function labelLoad(
  name: string,
): Promise<{ name: string; marks?: LabelMark[] }> {
  return request(`/labels/${encodeURIComponent(name)}`);
}
...
): Promise<{ name: string; marks: number; saved_at: string }> {
  return request(`/labels/${encodeURIComponent(name)}`, {
    method: "PUT",
    headers: JSON_POST,
    body: JSON.stringify(body),
...
export function labelList(): Promise<{
  sessions: { name: string; marks: number }[];
}> {
