...
export function Gate({
  ...
  if (who.held) {
    // 🔴 임시 보류 (사용자 2026-09-07) — 승인 없이 유예(기본 30일)가 지난 대기 계정. 아이들을 그리지 않는다:
  ...
  return (
    <Card title="임시 보류 상태입니다">
      ...
      <p className="m-0 text-xs text-blue-gray-500 dark:text-blue-gray-400">
        {who.email} — 가입 뒤 유예 기간
        {who.hold_after_hours
          ? `(${Math.round(who.hold_after_hours / 24)}일)`
          : ""}{" "}
        안에 승인되지 않아 접근이 보류됐다. 관리자가 승인하거나 보류를 풀면 바로
        들어올 수 있다.
      </p>
      {done ? (
