...
def tracked_markdown() -> list[Path]:
    """git 이 추적하는 마크다운 파일 — `public/` 덧씌우기는 뺀다.

    `public/` 의 링크는 내보낸 공개 트리 기준이라 이 저장소 안에서는 전부 끊긴 것으로 보인다
    (2026-09-08 · 53개). 그쪽은 `scripts/dev/check_public_readme.sh` 가 내보낸 트리에서 검사한다.
    """
    out = subprocess.run(
        ["git", "ls-files", "-z", "*.md"], cwd=ROOT, check=True, capture_output=True
    ).stdout
    return [ROOT / p for p in out.decode("utf-8").split("\0") if p and not p.startswith("public/")]
