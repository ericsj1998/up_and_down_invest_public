...
| `EDGAR_USER_AGENT` | Settings(`edgar_user_agent`) → `EdgarAdapter` | `이름 이메일` — SEC 요구. 없으면 재무 갱신(CLI · `POST /fundamentals/refresh`)이 즉시 실패하고 조회는 빈 표 | ⬜ 서버에 없음 — **사용자가 넣는다**(이메일이라 스크립트가 만들지 않는다 · T243) |
