...
```mermaid
flowchart LR
    G[구글 OAuth] --> C[세션 쪽지<br/>HMAC · 12h · secure/httpOnly] --> A[Account 행<br/>등급 + 권한 묶음 · 매 요청 DB 에서 읽음]
    A --> K{required_cap<br/>경로 · 메서드 · 서버}
    K -->|돈이 움직이는 경로| F[5분 재인증 FRESH_S<br/>+ 거래 기능]
    K -->|live 서버 · /exchange · /walkforward/live · /rebalancer| M[읽기도 실거래 조회 기능]
    K -->|설정을 못 읽으면| Z[조인다 — fail-closed<br/>on_real_money]
```
...
```mermaid
flowchart LR
    S[사용자 증상] --> N["① nginx 접근 로그<br/>브라우저가 실제 보낸 요청 · 상태 · 빈도 (499 는 여기만)<br/>status.sh browser traffic"]
    N --> J["② api JSON 로그<br/>event_type 집계 · trace_id → proposal → order → position<br/>docker logs · /admin/logs zip"]
    J --> R["③ /admin/resources<br/>psutil + cgroup · Redis 비트 · 레이트리밋 경로별 weight<br/>관리자 화면 자원 카드"]
    R --> V["④ vmstat steal · free · docker stats<br/>status.sh"]
```
