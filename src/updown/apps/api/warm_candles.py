...
- 축과 창: 차트 분석 주문의 단기(15m)·스윙(1h)은 `lookback_span`(정규장 기준 400+200봉),
  AI 비교의 4h 는 그쪽 `_lookback`(달력 400봉). 일봉은 순위표가 이미 매일 받는다.
...
def warm_spans() -> list[tuple[Timeframe, Callable[[Market], timedelta]]]:
    ...
    Returns:
        `(축, 시장 → 달력 기간)` 목록 — 15m·1h 는 차트 분석 주문 창, 4h 는 AI 비교 창.
    """
    return [
