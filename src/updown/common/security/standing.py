...
HOLD_AFTER = timedelta(days=30)
"""가입 뒤 이 시간 안에 승인되지 않으면 보류된다 (사용자 2026-09-07 "하루" → 2026-09-08 "한 달").

관리자가 화면에서 바꾸면 `app_settings` 값이 이것을 덮는다 — 이 상수는 설정이 없을 때의 기본이다.
"""
...
def contact_allowed(last_contact_at: datetime | None, now: datetime) -> bool:
    ...
    Args:
        last_contact_at: 마지막 문의 시각. None 이면 처음이다. naive 면 UTC 로 본다.
        now: 지금 (aware).
