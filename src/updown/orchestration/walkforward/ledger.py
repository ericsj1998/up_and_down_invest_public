...
class TradeRecord:
    ...
    """
    filled_leverage: Decimal | None = None
    """**체결이 실제로 만든 노출** (= 체결 명목 ÷ `sizing_base`) — 라이브가 체결 뒤 붙인다 (T288).

    `leverage` 는 **의도한** 배율이다. 계약은 정수라 실제로 산 것은 그것과 다르고(Gate 실측:
    SOL 계약 하나 111.55 USDT · DOGE 0.87 — 128배), v1.10.3 부터는 반올림이라 **더 살 수도**
    있다. 총 명목 상한(`open_exposure`)이 의도를 세면 두 방향으로 다 틀린다 —
    덜 산 만큼 방이 묶이고, **더 산 만큼은 상한을 넘겨도 모른다**(후자가 안전 쪽 결함이다).

    ⛔ **`leverage` 를 대신하지 않는다.** 손익률은 `leverage` 를 그대로 쓴다 — 결정론 코어가
    같은 입력에 같은 출력을 내야 하고(규칙 #5), 백테스트에는 정수 계약이 없다. 이 값은
    **상한 회계 전용**이다.

    None = 모형(백테스트·페이퍼)이거나 옛 기록 — 그때는 `leverage` 를 쓴다.
