...
class Session:
    ...
    **관통했는데 `filled=False` 인 항목이 곧 모형과 현실의 차이**다.
    ...
    """이 판을 띄운 **펀드 이름** — 단독 판이면 None (2026-09-26 · 계좌 예산 검사 면제).

    🔴 `fund_ready` 는 펀드가 문을 붙이면 참으로 돌아가 흔적이 사라진다 — 러너가
    "펀드 판인가" 를 물을 곳이 없었다. 러너의 `check_funding`(판 예산 합 > 계좌 → 전 판
    신규 진입 정지)은 사람이 판마다 예산을 정하던 단독 판의 장치인데, 펀드 판의 저장
    예산은 **판을 만든 날의 몫**이라 갱신되지 않는다. 그래서 펀드가 2% 만 잃어도 40판이
    전부 멈췄다(2026-09-25 17:30 UTC 실계좌 · 예산 합 417.31 > 계좌 406.82). 펀드 판은 이
    이름이 있으면 그 검사를 안 쓴다 — 크기는 이미 `_usable_equity`(예산 · 거래소 가용 중
    작은 쪽 x 0.99)가 계좌 안으로 잡는다.

    ⚠️ 꽂는 곳은 API 의 판 시작(`_live_start` — 펀드가 띄우거나 펀드 파일에 이 종목이
    있을 때) 하나다.
    """
